...
class SystemAPITestCase(unittest.TestCase):
    ...
    @mock.patch('akanda.router.utils.enabled_services')
    def test_get_configuration(self, fake_enabled_services):
        fake_enabled_services.return_value = ['router', 'loadbalancer']
    ...
    def test_put_configuration_with_adv_services(self, mock_update,
        ...
        fake_enabled_svc, fake_get_config_model):
        fake_enabled_svc.return_value= ['loadbalancer']
        fake_config.return_value = 'foo'
        fake_cache.return_value = 'fake_cache'
        sys_config_obj = mock.Mock()
        sys_config_obj.validate = mock.Mock()
